...
**This repo may need elevated permissions on GitHub (if a `project` node is  defined in [.tt_config.json](../.tt-config.json))**
...
GitHub Codespaces automatically injects a `GITHUB_TOKEN` environment variable containing a `ghu_***` (GitHub User) token but with some limited permissions. This token does **not** include `project:write` scope, which may be required by `devx-cafe/gh-tt`.
...
_N.B: This may not be the only solution — but the simplest_
...
gh auth login -h github.com -p https -s project -w  # stores the token in ~/.config/gh/hosts.yml 
```
...
### Step 2: Store Token so containers can read it
