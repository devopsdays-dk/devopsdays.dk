...
**When in agent mode**, creating intermediate or temporary files or scripts, always create them in the `./temp` folder. If it doesn't exist create the `temp` folder and check that it's ignored by git. If it's not already mentioned in the `.gitignore` file in the repo root, then add a `./temp/.gitignore` file containing only an asterisk (`*`).

## Branch names and the process we use

We do not use pull requests.

We generally use the GitHub CLI extension [gh-tt](https://github.com/devx-cafe/gh-tt) to manage our issues and branches. Most impotantly it supports `workon`, `wrapup` and `deliver`.
...
This will create a new branch named with the issue number as prefix (e.g. `321-Update_summary_instructions`) and switch to that branch. The `$PS1` is set up to show the branch in the prompt (e.g. `user ➜ /workspaces/gh-tt (321-Update_summary_instructions)` Issue branchs (development branches) are alwasy prefixed with an interger that referes to the the GitHub Issue number being worked on. In the example above we're working on issue 321).
...
When a CLI tool we usually do not like verbose informative statements. In general avoid adding print statements. If nothing happened, then nothing should be printed. I'll let you know explicitly when print statements are needed.
